...
---
This tool assumes you have access to Snowflake and can provide a unit/camera
IP address. It handles Axis and Hikvision cameras (including "LVT"-branded
units, which run Hikvision firmware). Dahua support is not yet implemented.
It will not take a snapshot of quad-view cameras.

# Camera Audit

Kicks off `camera_engine.run_batch()` through a script built for Claude to
drive: `scripts/run_audit.py` takes plain CLI arguments instead of
interactive menus or a Tkinter file picker.

This skill package is self-contained -- `camera_engine.py` ships inside
`scripts/` alongside `run_audit.py`. It does not depend on any other project
folder existing on the machine it runs on.

## Prerequisites (check this first on any new machine)

This skill shells out to a local `python` process -- it does not install
anything for the user. Before the first run on a machine:

1. Confirm `python` (3.11+) is on PATH: run `python --version`. If that
   fails, the user needs Python installed before this skill can do anything --
   don't try to work around a missing interpreter.
2. Confirm the dependencies from this skill's `requirements.txt` are
   installed. The cheapest check is just attempting a run -- if it fails with
   `ModuleNotFoundError`, that confirms it. If you want to check proactively,
   run `python -c "import requests, PIL, openpyxl"` in this skill's folder;
   a clean exit means dependencies are present.
3. If dependencies are missing, run (from this skill's folder):
   `pip install -r requirements.txt`
   Tell the user this is a one-time setup step per machine, not something
   that needs to happen on every audit.
...
```powershell
Start-Process powershell -ArgumentList "-NoExit", "-Command", "python '<skill_dir>/scripts/run_audit.py' --ip <ip> --manufacturer <axis|hikvision>"
```
...
If `Start-Process` itself errors out (rather than the launched script
failing), that's an environment problem, not a credentials problem -- e.g.
PowerShell execution policy blocking script launches, or `python` not being
resolvable on PATH from a fresh shell. Surface the exact error to the user
instead of retrying blindly; don't assume it's the same 401/lockout pattern
covered in `references/troubleshooting.md`, since that document covers
failures *after* the script starts running.
